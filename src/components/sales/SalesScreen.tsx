...
import type { CustomerDto, InvoiceDetailDto, ProductDto, SaleHistoryDto, SaleInput } from "@/types/stylecraft-api";
...
function makeItem(rowId: number): ItemFormState {
  return { rowId, productId: "", quantity: "1", unitPrice: "0.00", discount: "0.00" };
}
...
function buildSaleInput(form: SaleFormState): { input?: SaleInput; error?: string } {
  ...
  for (const item of form.items) {
    ...
    if (!Number.isInteger(productId) || productId <= 0) {
      return { error: "Select a product for every sale item." };
    }
    ...
    if (seenProductIds.has(productId)) {
      return { error: "Each product can appear only once per sale." };
    }

    seenProductIds.add(productId);
    items.push({ productId, quantity, unitPriceCents, discountAmountCents });
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-sm font-medium text-blue-600">Milestone 5 sales module</p>
          <h2 className="mt-1 text-3xl font-bold tracking-tight">Sales</h2>
          <p className="mt-2 max-w-2xl text-slate-600">Record sales, block overselling, calculate profit, decrease stock, and produce readable invoices.</p>
        </div>
      ...
      <form className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4" noValidate onSubmit={handleSubmit}>
        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
          <div>
            <h3 className="font-semibold text-slate-950">Create sale</h3>
            <p className="mt-1 text-sm text-slate-500">Each saved item decreases stock and records a stock movement.</p>
          </div>
          <div className="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-950 shadow-sm">Total {formatCurrency(totalCents)}</div>
        </div>

        <div className="grid gap-4 md:grid-cols-4">
          <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor="sale-customer">
            Customer
            <select
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor="sale-new-customer">
            New customer name
            <input
              className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor="sale-date">
            Sale date
            <input
              className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              id="sale-date"
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor="sale-payment-method">
            Payment method
            <input
              className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
        ...
        <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor="sale-notes">
          Notes
          <textarea
        ...
        <div className="space-y-3">
          ...
          {form.items.map((item, index) => (
            <div
              className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-[minmax(0,1fr)_120px_140px_140px_120px_auto]"
              key={item.rowId}
            >
              <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor={`sale-product-${item.rowId}`}>
                Product {index + 1}
                <select
              ...
              <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor={`sale-quantity-${item.rowId}`}>
                Quantity
                <input
                  className="rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              ...
              <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor={`sale-unit-price-${item.rowId}`}>
                Unit price
                <input
                  className="rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              ...
              <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor={`sale-discount-${item.rowId}`}>
                Discount
                <input
                  className="rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200">
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">
          <h3 className="font-semibold text-slate-950">Sale history</h3>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {isLoading ? (
                ...
                sales.map((sale) => (
                  <tr key={sale.id}>
                    <td className="px-4 py-4 font-medium text-slate-950">{sale.invoiceNumber}</td>
                    <td className="px-4 py-4 text-slate-600">{formatDate(sale.saleDate)}</td>
                    <td className="px-4 py-4 text-slate-600">{sale.customerName ?? "No customer"}</td>
                    <td className="px-4 py-4 text-slate-600">{sale.itemCount}</td>
                    <td className="px-4 py-4 font-semibold text-slate-950">{formatCurrency(sale.totalAmountCents)}</td>
                    <td className="px-4 py-4 font-semibold text-slate-950">{formatCurrency(sale.profitAmountCents)}</td>
                    <td className="px-4 py-4 text-slate-600">{sale.paymentMethod ?? "-"}</td>
                    <td className="px-4 py-4 text-slate-600">{sale.notes ?? "-"}</td>
      ...
      {selectedInvoice ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-label="Invoice preview">
          <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-sm font-medium text-blue-600">Milestone 8 invoice preview</p>
              <div className="mt-2 flex items-center gap-3">
                <img alt="StyleCraft logo" className="h-12 w-12 rounded-xl border border-slate-200 bg-white p-1" src={styleCraftLogoDataUri} />
                <h3 className="text-2xl font-bold text-slate-950">Invoice {selectedInvoice.invoiceNumber}</h3>
              </div>
              <p className="mt-1 text-sm text-slate-500">{formatDate(selectedInvoice.saleDate)}</p>
            </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl bg-slate-50 p-4">
              <h4 className="font-semibold text-slate-950">Business</h4>
              <p className="mt-2 font-medium text-slate-800">{selectedInvoice.business.businessName}</p>
              <p className="text-sm text-slate-600">{selectedInvoice.business.phone ?? "No phone"}</p>
              <p className="text-sm text-slate-600">{selectedInvoice.business.email ?? "No email"}</p>
              <p className="text-sm text-slate-600">{selectedInvoice.business.address ?? "No address"}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <h4 className="font-semibold text-slate-950">Customer</h4>
              <p className="mt-2 font-medium text-slate-800">{selectedInvoice.customerName ?? "No customer"}</p>
              <p className="text-sm text-slate-600">{selectedInvoice.customerPhone ?? "No phone"}</p>
              <p className="text-sm text-slate-600">{selectedInvoice.customerEmail ?? "No email"}</p>
              <p className="text-sm text-slate-600">{selectedInvoice.customerAddress ?? "No address"}</p>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {selectedInvoice.items.map((item) => (
                  <tr key={item.id}>
                    <td className="px-4 py-4 font-medium text-slate-950">{item.productName}</td>
                    <td className="px-4 py-4 text-slate-600">{item.productSku}</td>
                    <td className="px-4 py-4 text-slate-600">{item.quantity}</td>
                    <td className="px-4 py-4 text-slate-600">{formatInvoiceCurrency(item.unitPriceCents, selectedInvoice.business.currencySymbol)}</td>
                    <td className="px-4 py-4 text-slate-600">{formatInvoiceCurrency(item.discountAmountCents, selectedInvoice.business.currencySymbol)}</td>
                    <td className="px-4 py-4 font-semibold text-slate-950">{formatInvoiceCurrency(item.totalAmountCents, selectedInvoice.business.currencySymbol)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
